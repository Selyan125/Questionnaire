...
apiRouter.delete('/categories/:id', requireAdmin, async (req, res) => { const id = Number(req.params.id); const qs = await prisma.question.findMany({ where: { questionnaire: { id } }, select: { id: true } }); for (const q of qs) await prisma.questionElement.deleteMany({ where: { questionId: q.id } }); await prisma.question.deleteMany({ where: { questionnaire: { id } } }); await prisma.questionCategory.delete({ where: { id } }); res.status(204).end() })
apiRouter.post('/categories/:id/questions', requireAdmin, async (req, res) => { res.status(201).json(await prisma.question.create({ data: { title: req.body.title, questionnaire: { connect: { id: Number(req.params.id) } } } })) })
apiRouter.put('/questions/:id', requireAdmin, async (req, res) => { const { title, questionCategoryId } = req.body; const data = { title }; const cid = Number(questionCategoryId); if (!isNaN(cid)) data.questionnaire = { connect: { id: cid } }; res.json(await prisma.question.update({ where: { id: Number(req.params.id) }, data })) })
