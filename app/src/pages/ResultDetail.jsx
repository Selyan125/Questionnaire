    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 2 }}>
        {loading ? <Typography>Chargement...</Typography> : error ? <Typography color="error">{error}</Typography> : (
          <>
            <Typography sx={{ fontWeight: 700, mb: 1 }}>Élément</Typography>
            <Typography sx={{ mb: 2 }}>{result && JSON.stringify(result, null, 2)}</Typography>

            {result && result.answers && Array.isArray(result.answers) && (
              <Table>
                <TableHead>
                  <TableRow>
                ...
                <TableBody>
                  {result.answers.map((a, i) => (
                    <TableRow key={i}>
                      <TableCell>{a.questionTitle || a.question || a.q || '—'}</TableCell>
                      <TableCell>{Array.isArray(a.answer) ? a.answer.join(', ') : (a.answer ?? '')}</TableCell>
                      <TableCell>{a.score ?? a.points ?? '—'}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
